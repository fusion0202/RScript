...
df <- read.csv("https://raw.githubusercontent.com/fusion0202/RScript/master/covid_chiba.csv", check.names = FALSE)
...
cap <- "Data Source: https://github.com/fusion0202/RScript/blob/master/covid_chiba.csv"
...
ggplot(data = map) +
  ...
  theme(legend.position = c(0.80, 0.05)) +
  labs(title = "COVID-19 Cases in Chiba",
       subtitle = paste("from ", as.character(as.Date(cDate) - Days + 1), " to ", cDate),
       caption = cap)
